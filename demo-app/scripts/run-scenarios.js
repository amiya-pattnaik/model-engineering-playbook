...
import { predictRiskCart } from '../src/services/predictor-cart.js';
...
function loadScenarios(args) {
  const scenarioArgs = args.filter((arg) => !arg.startsWith('--engine='));
...
function runScenario(filePath, model) {
  ...
  const prediction = engine === 'cart'
    ? predictRiskCart(model, scenario.change)
    : predictRisk(model, scenario.change);
  ...
  const baseName = `${run.scenario}${engine === 'cart' ? '-cart-v2' : ''}`.replace(/\s+/g, '-').toLowerCase();
  ...
  console.log(`${pass === false ? 'x' : '✓'} ${run.scenario} [${engine}]: ${prediction.risk} (${prediction.confidence})`);
}
